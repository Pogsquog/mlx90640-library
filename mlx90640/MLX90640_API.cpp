...
//------------------------- To calculation -------------------------------------    
  ...
  for (int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
  {
    ...
    conversionPattern =
      ((pixelNumber + 2) / 4 - (pixelNumber + 3) / 4 + (pixelNumber + 1) / 4 - pixelNumber / 4) * (1 - 2 * ilPattern);
    ...
    if (pattern == frameData[833])
    {
      ...
      alphaCompensated =
        (params->alpha[pixelNumber] - params->tgc * params->cpAlpha[subPage]) * (1 + params->KsTa * (ta - 25));
      ...
      To = sqrt(sqrt(
        irData / (alphaCompensated * alphaCorrR[range] * (1 + params->ksTo[range] * (To - params->ct[range]))) + taTr))
        - 273.15;
...
void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
  ...
  for (int i = 0; i < 24; i++)
  {
    for (int j = 0; j < 32; j++)
    {
      ...
      mlx90640->alpha[p] =
        (alphaRef + (accRow[i] << accRowScale) + (accColumn[j] << accColumnScale) + mlx90640->alpha[p]);
